...
class TaxYearCalcs
{
   public function taxDueOnIncomeByYear(\App\Entity\ProjectSpecific\TaxYear $taxYear, float $income){
    ...
    public function taxReliefsByYear(\App\Entity\ProjectSpecific\TaxYear $taxYear ){
        $tax_input_by_year = $this->taxInputsRepository->findOneBy(['year'=>$taxYear]);
        $tax_relief_offset = 0;
    ...
    public function incomeOffsetsByYear(\App\Entity\ProjectSpecific\TaxYear $taxYear ){
        $tax_input_by_year = $this->taxInputsRepository->findOneBy(['year'=>$taxYear]);
        $income_offset = 0;
        foreach ($this->investmentRepository->findAll() as $investment ){
            if($investment->getEisPurchaseYear1()){
